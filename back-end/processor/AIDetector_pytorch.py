...
class Detector(object):
    ...
    def init_model(self):
        ...
        self.m = YOLO(r'C:\projects\guangzhou\ultralytics\runs\detect\train8\weights\best.pt')  
        # self.names = model.module.names if hasattr(
        #     model, 'module') else model.names
        self.names = ['jump','fall','leaking','knocking']
        self.colors = [
    ...
    def detect(self, im):
        ...
        #             image_info[key] = ['{}×{}'.format(git add .
        #                 x2-x1, y2-y1), np.round(float(conf), 3)]

        # im = self.plot_bboxes(im, pred_boxes)


        results = self.m(im)  # return a list of Results objects
        pred_boxes = []
        image_info = {}
        count = 0
        ...
        for result in results:
            ...
            for *x, conf, cls_id in boxes.data:
                lbl = self.names[int(cls_id)]
                lbl = int(cls_id)
                x1, y1 = int(x[0]), int(x[1])
                x2, y2 = int(x[2]), int(x[3])
                pred_boxes.append(
                    (x1, y1, x2, y2, lbl, conf))
                count += 1
                key = '{}-{}'.format(self.names[lbl],count)
